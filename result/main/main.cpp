...
void Final(double**Final)
{
	...
	for (int i = 0; i <= Sstep; i++)
	{
		if (i > (Trigger * Muti))
		{
			Final[i][Tstep] = max(1000 + 1000 * (Strike - i / Muti) / Strike, 0);
		}
		else
		{
			Final[i][Tstep] = 1031.375;
		}
	}
}
void Midway(double**Former,double**Later)
{
	for (int i = 0; i <= Sstep; i++)
	{
		for (int j = 0; j <= Tstep; j++)
...
int main(int argc, char* argv[])
{
	...
	Midway(Stage4, Stage3);
	FiniteDifference(Stage3);
	Midway(Stage3, Stage2);
	FiniteDifference(Stage2);
	Midway(Stage2, Stage1);
